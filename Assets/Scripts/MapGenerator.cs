...
public class MapGenerator : MonoBehaviour {
	...
	private List<Coord> GetLine(Coord from, Coord to){
		...
		bool inverted = false;
		int step = (int) Mathf.Sign(float.Parse(dx.ToString()));
		int gradientStep = (int) Mathf.Sign(float.Parse(dy.ToString()));

		int longest = (int) Mathf.Abs(float.Parse(dx.ToString()));
		int shortest = (int) Mathf.Abs(float.Parse(dy.ToString()));
	...
	private void ProcessMap() {
		...
		foreach (List<Coord> wallRegion in roomRegions) {
			if (wallRegion.Count < roomThresholdSize) {
				...
				survivingRooms.Add(new Room(wallRegion, map));
			}
		}
